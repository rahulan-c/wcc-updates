# ============================================================================= 
#            WORLD CHESS CHAMPIONSHIP MATCH VISUSALISATIONS
# =============================================================================

# Inspired by Simran Parwani's graphics for FiveThirtyEight in 2021: 
# https://53eig.ht/3EJG2ln.

# Optimised for a 4-column layout

# TODO (maybe)
# - add plot explanation element to left of today's game plot
# - resize player colour indicators in top plot
# - add move annotation symbols? (!, ? etc)
...
annotations_12 <- tibble::tibble(
  ...
  comment = c(
    ...
    "Nepo isn't punished<br>for the bizarre <b><span style = 'color:#006994;'>Bd7</span></b>.",
    "Ding cracks first with <b><span style = 'color:#922724;'>Bc2</span></b>,<br>giving Black an advantage.",
    "With the strong <b><span style = 'color:#006994;'>b5</span></b>, Nepo looks<br>odds-on to secure a win after a virtually<br> flawless attacking display, but his next<br>move <b><span style = 'color:#006994;'>b4</span></b> is a serious practical mistake.",
    "Nepo gives Ding a huge chance<br>with <b><span style = 'color:#006994;'>Bb8</span></b>, but Ding returns it with<br><b><span style = 'color:#922724;'>Qb7</span></b> - a double blunder!",
    "<b><span style = 'color:#006994;'>f5</span></b> instantly loses the game!<br>Nepo is visibly wracked in<br>anguish. He can't believe<br>what he's just done.", 
    "Nepo resigns, tying<br>the overall score.<br>What a game!"
    ),
  x_adj = c(0, 0, 0, 0, 0, 0, 0.5), # moves
  y_pos = c(1, -1.2, 2.1, -0.5, 2.6, 1.4, -1.3), # eval
  arrow_y = c(0, -0.8, 1.3, 0, 1.4, 0, 0),
  arrow_yend = c(0.2, -1.15, 1.6, -0.3, 1.7, 0.3, 0),
  x_just = c(0, 0.2, 1, 0.85, 0, 0, 0),
  y_just = c(1, 1, 1, 1, 1, 1, 1)
)
...
score_538 <- grid::grobTree(
  ...
  grid::textGrob(
    label = glue("Current score after 13 games"),
    x = 0.5, y= 0.75,
    hjust = 0.5,
    vjust = 0.5,
    gp = grid::gpar(
...
# graphic <- patchwork::wrap_plots(A = explainer, 
#                                  B = today_element, 
#                                  C = score_element,
#                                  D = all_games,
#                                  E = footer,
...
now_label <- paste0(lubridate::ymd(
  lubridate::today("UTC")), 
  "_", 
  lubridate::hour(lubridate::now("UTC")), 
  lubridate::minute(lubridate::now("UTC")
                    ))
